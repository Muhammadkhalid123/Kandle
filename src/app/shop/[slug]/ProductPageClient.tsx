...
import { Star, ShoppingBag, ArrowLeft, Heart, Share2, Check } from "lucide-react";
import Link from "next/link";
import { notFound } from "next/navigation";
import { useState } from "react";
import { useCart } from "@/context/CartContext";
import { shareContent } from "@/lib/utils";
import { getRelatedBooks } from "@/lib/seo/internal-linking";
...
export default function ProductPageClient({ book }: ProductPageClientProps) {
    ...
    const relatedBooks = getRelatedBooks(book, 4);
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                    ...
                    <div className="lg:col-span-5 space-y-6">
                        <div>
                            ...
                            <div className="flex items-center gap-4 mb-4" itemScope itemType="https://schema.org/AggregateRating">
                                <meta itemProp="ratingValue" content={book.rating.toString()} />
                                <meta itemProp="reviewCount" content={book.reviews.toString()} />
                ...
                {relatedBooks.length > 0 && (
                    <section className="mt-16">
                        <h2 className="text-3xl font-bold mb-8">Customers Also Bought</h2>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                            {relatedBooks.map((relatedBook) => (
                                ...
                                >
                                    <meta itemProp="name" content={relatedBook.title} />
                                    <meta itemProp="url" content={`https://kandledirectpublishing.com/shop/${relatedBook.slug}`} />
                                    ...
                                    <h3 className="font-bold text-sm mb-1 group-hover:text-accent transition-colors line-clamp-2">{relatedBook.title}</h3>
                                    <p className="text-gray-400 text-xs mb-1">{relatedBook.author}</p>
                                    <p className="text-white font-bold text-sm">${relatedBook.price}</p>
                                </Link>
                            ))}
                        </div>
                    </section>
